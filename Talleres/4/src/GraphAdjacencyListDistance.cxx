...
Node<Airport>* GraphAdjacencyListDistance::getAdjListNode(Edge<Flight> destination, double distance, Node<Airport>* head_node) {
...
GraphAdjacencyListDistance::GraphAdjacencyListDistance(std::vector<Flight*> flights, std::unordered_map<int, Airport*> airports) {
    ...
    for (int k = 0; k < num_edges; k++) {
        ...
        double distance = calcDistance(src->latitude, src->longitude, dst->latitude, dst->longitude);
        // std::cout << std::setprecision(10) << src->latitude << ", " << src->longitude << ". " << dst->latitude << ", " << dst->longitude<< " -- " << distance << std::endl;

        Node<Airport>* new_node = getAdjListNode(destination, distance, head_node[source_index]);
        head_node[source_index] = new_node;
    }
}

GraphAdjacencyListDistance::~GraphAdjacencyListDistance() {
...
void GraphAdjacencyListDistance::print() {
    for (int i = 0; i < number_of_nodes; i++) {
...
double GraphAdjacencyListDistance::calcDistance(double x1, double y1, double x2, double y2) {
    return sqrt(pow(x2 - x1, 2) + pow(y2 - y1, 2));
}

std::vector<Airport*> GraphAdjacencyListDistance::shortestPath(const std::string& source_airport_code, const std::string& destination_airport_code) {
